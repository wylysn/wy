...
//            sqlite3_bind_text(statement, 4, [device.Class UTF8String], -1, SQLITE_TRANSIENT);
            ...
            sqlite3_bind_text(statement, 14, [device.Attachment?device.Attachment:@"" UTF8String], -1, SQLITE_TRANSIENT);
            sqlite3_bind_text(statement, 15, [device.Organizationid?device.Organizationid:@"" UTF8String], -1, SQLITE_TRANSIENT);
...
- (NSArray *)findAllDevices {
    ...
    const char *dbpath = [databasePath UTF8String];
    if (sqlite3_open(dbpath, &database) == SQLITE_OK) {
        NSString *querySQL = [NSString stringWithFormat: @"select Code, Name, Model, Brand, Xlhao, Pro_Date, Weight, Des_Life, Pos, Description, Patrol_Tpl, Status, Picture, Attachment, Organizationid, ClassName, PosID, ClassID from Device"];
        const char *query_stmt = [querySQL UTF8String];
        if (sqlite3_prepare_v2(database, query_stmt, -1, &statement, NULL) == SQLITE_OK)
        {
            while (sqlite3_step(statement) == SQLITE_ROW) {

...
- (void)viewWillAppear:(BOOL)animated
{
	...
	dispatch_async( self.sessionQueue, ^{
		switch ( self.setupResult )
		{
			...
			{
				dispatch_async( dispatch_get_main_queue(), ^{
					NSString *message = NSLocalizedString( @"您没有权限访问相机，请去设置", @"用户拒绝使用相机警告" );
					UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"相机" message:message preferredStyle:UIAlertControllerStyleAlert];
					...
					UIAlertAction *settingsAction = [UIAlertAction actionWithTitle:NSLocalizedString( @"设置", @"Alert button to open Settings" ) style:UIAlertActionStyleDefault handler:^( UIAlertAction *action ) {
			...
			{
				dispatch_async( dispatch_get_main_queue(), ^{
					NSString *message = NSLocalizedString( @"启动相机失败", @"Alert message when something goes wrong during capture session configuration" );
					UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"相机" message:message preferredStyle:UIAlertControllerStyleAlert];
...
- (IBAction)resumeInterruptedSession:(id)sender
{
	dispatch_async( self.sessionQueue, ^{
		...
		if ( ! self.session.isRunning ) {
			dispatch_async( dispatch_get_main_queue(), ^{
				NSString *message = NSLocalizedString( @"不能重置照片", @"Alert message when unable to resume the session running" );
				UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"相机" message:message preferredStyle:UIAlertControllerStyleAlert];
				UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:NSLocalizedString( @"确定", @"Alert OK button" ) style:UIAlertActionStyleCancel handler:nil];
				[alertController addAction:cancelAction];
				[self presentViewController:alertController animated:YES completion:nil];
...
//                    [_delegate showImageDate:imageData];
//                }
...
- (IBAction)cacelCamera:(id)sender {
